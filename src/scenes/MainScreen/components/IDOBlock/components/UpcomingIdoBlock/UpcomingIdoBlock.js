...
export function UpcomingIdoBlock({ props }) {
  const [seconds, setSeconds] = useState(
    typeof props.saleInfo?.time_until_launch === "string"
      ? 0
      : props.saleInfo?.time_until_launch ?? 0
  );
  ...
  return (
    <div className={classes.IdoBlock}>
      {!!props.blank_img && (
        <div
          className={classes.blankIDO}
          style={{ backgroundImage: `url(${props.blank_img})` }}
        >
          <h1>TBA</h1>
        </div>
      )}
          ...
          <main>
            <div className={classes.saleInfo}>
              <div className={classes.privateSaleFlag}>
                {props.title == "Another-1"  ? "Pre-sale" : props.is_private_sale ? "Private Sale" : "Public Sale"}
              </div>
              ...
              {activeSection === 0 && (
                <div className={classes.offeringSection}>
                  ...
                  <div className={classes.textToShowBlock}>
                    {textToShow(
                      "Start Date",
                      props.timeline.show_text
                        ? props.timeline.sale_timeline_text
                        : new Date(
                            props.timeline.registration_start * 1000
