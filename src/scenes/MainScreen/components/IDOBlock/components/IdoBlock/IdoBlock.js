...
export function IdoBlock({ props }) {
  ...
  const [seconds, setSeconds] = useState(
    props.timeline.sale_end - date < 0 ? 0 : props.timeline.sale_end - date
  );
  const dispatch = useDispatch();

  const [totalBUSDRaised, setTotalBUSDRaised] = useState(0);
  const [saleProgress, setSaleProgress] = useState(0);
  const { contract } = useJSONContract(props.sale_contract_address, SALE_ABI);
  ...
  useEffect(() => {
    setSaleProgress(
      (100 * totalBUSDRaised) /
        props.token.price /
        props.token.token_distribution
    );
  }, [totalBUSDRaised]);
      ...
      <header>
        ...
        <div className={classes.tokenBlock}>
          <div className={classes.progresLabel}>
            {false &&
              props.timeline.sale_start * 1000 < Date.now() &&
              props.timeline.sale_end * 1000 > Date.now() && (
                <div className={classes.styledLabel}>In Progress</div>
              )}
          </div>
        </div>
      </header>
      ...
      <main>
        <div className={classes.saleInfo}>
          ...
          <div className={classes.textToShowBlock}>
            ...
            {textToShow(
              "Token Price",
              isNaN(props.token.price)
                ? "TBA"
                : priceToFormatedPrice(props.token.price)
            )}
          </div>
    ...
    return (
      <div className={classes.totalRaised}>
        ...
        <div className={classes.count}>
          ...
          /${numberWithCommas(props.price * props.token_distribution)}
        </div>
      </div>
    );
  } else {
