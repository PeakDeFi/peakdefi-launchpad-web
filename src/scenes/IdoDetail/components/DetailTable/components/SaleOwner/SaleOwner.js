...
const SaleOwner = ({ ido, saleContract }) => {
  ...
  return (
    <div className={classes.SaleOwner}>
      <h1>Sale owner operations</h1>
      <section>
        <p>Withdraw Earnings:</p>
        <Button
          isActive={ido.timeline.sale_end * 1000 < Date.now()}
          onClick={withdrawEarningsHander}
          text="Withdraw"
        />
      </section>
      ...
      <section>
        ...
        <Button
          isActive={ido.timeline.sale_end * 1000 < Date.now()}
          onClick={withdrawLeftoverHandler}
          text="Withdraw"
        />
      </section>
      ...
      <section>
        <p>Deposit Tokens:</p>
        <Button
          isActive={ido.timeline.sale_start * 1000 > Date.now()}
          onClick={
            parseInt(ido.target_raised) * (10 ^ parseInt(ido.token.decimals)) >
            allowance
