...
export function MainInfo(props) {
  ...
  useEffect(async () => {
    ...
    if (userWalletAddress && ethereum) {
      ...
      ltokenContract
        .allowance(userWalletAddress, props.ido.contract_address)
        .then((response) => {
      ...
      saleContract
        ...
        .catch((error) => {
          if (error.data.message.includes("Need to stake minimum")) {
            setShowError(true);
            setErrorMessage(
              "You need to stake minimum amount of 10000 PEAK before registering for sale"
            );
          }
        });
    ...
    try {
      if (amount < 10) {
        ...
        let bigAmount = BigNumber.from(Math.round(roundedAmount * 100)).mul(
          BigNumber.from(10).pow(props.ido.token.decimals - 2)
        );
        saleContract
  ...
  const approve = async () => {
    try {
      tokenContract
        .approve(props.ido.contract_address, ethers.constants.MaxUint256)
        .then((response) => {
